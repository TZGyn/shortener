<script lang="ts">
	...
	let editData: typeof $page.state.newEditProjectLink
	...
	const showEditModal = async () => {
		...
		if (result.type === 'loaded' && result.status === 200) {
			editData = result.data as typeof $page.state.newEditProjectLink
		...
		<Card.Title class="flex items-center justify-between gap-4">
			...
			<DropdownMenu.Root>
				...
				<DropdownMenu.Content>
					<DropdownMenu.Group>
						<a
							href={`/dashboard/links/${shortener.id}/edit`}
							onclick={(event) => {
		...
		<div class="flex items-center justify-between">
			<div class="flex gap-2">
				...
				<a
					...
					href={`${getUrl()}/links/${shortener.id}/qr`}
					onclick={(event) => {
						event.preventDefault()
						showQRModal(event)
					}}>
